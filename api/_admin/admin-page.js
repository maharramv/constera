...
const privateAdminScripts = new Set([
  "admin-v2.js",
  "launch-center.js",
  "enterprise-admin.js",
...
const adminHtml = rendered
  ...
  .replace(
    ...
    (_match, prefix, file, suffix) =>
      `${prefix}/api/admin-download?file=${encodeURIComponent(file)}${suffix}`
  );
...
export default async function adminPageHandler(req, res) {
  ...
  try {
    ...
    return send(res, 503, "İdarəetmə sessiyası hazırda yoxlanıla bilmir.");
  }

  if (!user) {
    res.statusCode = 302;
    res.setHeader("Location", "/login.html?next=admin.html");
    return res.end();
  }
  if (!["super_admin", "admin"].includes(user.role)) {
    return send(res, 403, "Bu səhifəyə giriş icazəniz yoxdur.");
  }
